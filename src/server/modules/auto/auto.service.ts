...
export class AutoService {
  ...
  async makes(): Promise<string[]> {
    return await this.autoRepository.query('SELECT DISTINCT make from auto ORDER BY make')
  }

  async makeModels(makeTitle: string): Promise<string[]> {
    return await this.autoRepository.query(
        `SELECT DISTINCT model from auto WHERE make = '${makeTitle}' ORDER BY model`
    )
  }
}
